...
*/
#define MON_BUS_TIME_TO_IDLE 8400
#define MON_BUS_COL_DET_TIME 530
TimerOne timer;
...
void MON_RX_LOW_TO_HIGH() {
  if (( monitor_state == MON_BUSY )|| (monitor_state == MON_IDLE)){
...
void MON_RX_HIGH_TO_LOW() {
  if ( (monitor_state == MON_BUSY) || (monitor_state == MON_COLL)){
...
void setTimer(double millisec) {
  timer.attachInterrupt(timerISR, millisec); //library uses us not ms
}
